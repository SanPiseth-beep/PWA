import { savePalette } from './indexedDB.js';
import { createPalette } from './firebaseCRUD.js';
...
function displaySwatches(colors) {
  ...
  // Save the generated palette to IndexedDB
  const palette = { colors };
  savePalette(palette).then((savedPalette) => {
    // Save the palette to Firestore if online
    if (navigator.onLine) {
      createPalette(savedPalette);
    }
    // Update the UI with the new palette
